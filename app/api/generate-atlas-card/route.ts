...
async function analyzeObject(imageBase64: string, modelId: string, byok?: ByokParams) {
  const content = await chatCompletion(modelId, [
    { role: "system", content: "你是一个精准的物体识别器。识别照片中最主要的现实物体，并从固定分类中选择最合适的一个。" },
    { role: "user", content: [
      { type: "text", text: `识别这张照片中最主要的现实物体。返回 JSON：{ "objectName": "具体名称", "category": "分类" }。可选分类：${CATEGORY_LIST}。只返回 JSON。` },
  ...
  if (!CATEGORIES.includes(parsed.category as typeof CATEGORIES[number])) {
    const match = CATEGORIES.find(c => c === parsed.category || c.includes(parsed.category) || parsed.category.includes(c));
...
规则
- fantasyName：使用真实物品名称，不要幻想名
- description：客观、自然、清楚，不要幻想化，不要中二，不要城市特色
- stats：恰好3条，2条numeric + 1条text，要现实合理
- funFact：真实冷知识或科普小知识，不要城市梗或幻想梗
- 只输出 JSON`
    : `请为以下物体撰写一张图鉴卡片。
背景
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const content = await generateCardContent(styleProfile, analysis.objectName, analysis.category, effectiveModelId, plain, byok);
    return NextResponse.json({ card: { id: uid(), city: styleProfile.name, originalObjectName: analysis.objectName, category: analysis.category, fantasyName: content.fantasyName, description: content.description, stats: content.stats, funFact: content.funFact, imageUrl: image, createdAt: new Date().toISOString() } satisfies AtlasCard });
  } catch (e) {
    if (e instanceof ApiError) return NextResponse.json({ error: e.message, details: e.details }, { status: e.status });
    if (e instanceof SyntaxError) return NextResponse.json({ error: "AI 返回了无效的 JSON，请重试" }, { status: 502 });
    if (e instanceof ZodError) return NextResponse.json({ error: "AI 返回的数据格式不符合要求，请重试" }, { status: 502 });
    throw e;
  }
}
